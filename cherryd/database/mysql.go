...
func (r *MySQL) UpdateHAProxy(vipID uint64, haproxy network.HAProxyParam) error {
	f := func(db *sql.DB) error {
		tx, err := db.Begin()
		if err != nil {
			return err
		...
		haproxyID, err := updateHAProxy(tx, vipID, haproxy)
		if err != nil {
			return err
		}
		if err := updateBackends(tx, haproxyID, haproxy.Backends); err != nil {
			return err
		}
